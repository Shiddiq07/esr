...
class MY_Model extends MY_Orm {
        ...
        if ($where !== NULL) {
            if (is_array($where)) {
                foreach ($where as $field=>$value) {
                    (is_array($value)) ? $this->_dbr->where_in($field, $value) : $this->_dbr->where($field, $value);
                    // $this->_dbr->where($field, $value);
                }

            } else {
                $this->_dbr->where($this->primaryKey, $where);
            }
        }

        if ($this->soft_delete === true) {
            $this->_dbr->where(['deleted_at' => null]);
        }

        $result = $this->_dbr->get()->row();
        if ($result) {
            return $result;
    ...
    public function getAll($where = array(), $limit = null) {
        ...
        if ($limit) {
            $this->_dbr->limit($limit);
        }

        $result = $this->_dbr->get()->result();
        if ($result) {
            return $result;
        } else {
            return [];
    ...
    //     if ($this->_dbr->insert($this->tableName, $data)) {
    //         return $this->_dbr->insert_id();
    //     } else {
    //         return false;
    //     }
    // }
    ...
    public function insertBatch(Array $data) {
        $this->_dbr->insert_batch($this->tableName, $data);
        
        if($this->_dbr->affected_rows() < 1){
            return false;
        } else {
            return true;
        }
    ...
    //     return $this->_dbr->update($this->tableName, $data, $where);
    // }
    ...
    public function delete($where = [], $force = false) {
        ...
        if ($this->soft_delete === true && $force === false) {
            $this->update([
                'deleted_at' => date('YmdHis'),
                'deleted_by' => $this->session->userdata('identity')['id'],
            ], $where, false);
        } else {
            $this->_dbr->delete($this->tableName, $where);
        }

        return $this->_dbr->affected_rows();
    }
    ...
    protected function _get_datatables_query()
    {
        ...
        if(isset($_POST['order'])) {
            ...
            $this->_dbr->order_by(key($order), $order[key($order)]);
    ...
    function get_datatables()
    {
        ...
        if($_POST['length'] != -1) {
        	$this->_dbr->limit($_POST['length'], $_POST['start']);
        }

        $query = $this->queryAll($this->_dbr);
