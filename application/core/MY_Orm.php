...
class MY_Orm extends CI_Model// implements \ArrayAccess
{
    ...
    public function update($attributes, $condition=NULL, $runValidation=true)
    {
        ...
        $sql = $this->_dbr->set($attributes)->get_compiled_update('', false);
        $previous_values = $query->get()->row_array();
        $this->_dbr->reset_query();

        $status = $this->_db->query($sql);

        // Add to log audit trail
        // $this->trail($status, 'update', $this->tableName, $attributes, $previous_values);
    ...
    /**
     * Active Record transform to object record
     *
     * @return array
     * @example $record = $activeRecord->toObject();
     */
    public function toObject()
    {
        return (object) $this->_readProperties;
    }
    ...
    public function trail($status, $event, $table, $set = NULL, $previous_values = NULL)
    {
        ...
        if(!get_instance()->config->item('audit_enable')) return 1; // trail not enabled
        if($event === 'insert' && !get_instance()->config->item('track_insert')) return 1; // insert tracking not enabled
        if($event === 'update' && !get_instance()->config->item('track_update')) return 1; // update tracking not enabled
        if($event === 'delete' && !get_instance()->config->item('track_delete')) return 1; // delete tracking not enabled
        if(in_array($table, get_instance()->config->item('not_allowed_tables'))) return 1; // table tracking not allowed
        ...
        $id_user = def(get_instance()->session->userdata('identity'), 'id', 0);

        return $this->_db->insert('sys_audit_trails', [
            'user_id' => $id_user,
            'event' => $event,
            'table_name' => $table,
            'old_values' => $old_value,
            'new_values' => $new_value,
            'url' => get_instance()->uri->ruri_string(),
            'ip_address' => get_instance()->input->ip_address(),
            'user_agent' => get_instance()->input->user_agent(),
