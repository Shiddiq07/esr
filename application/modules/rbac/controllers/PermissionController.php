        ...
        foreach ($list as $field) {
            ...
            $row[] = "
            	<div class='text-center'>
	            	". anchor("/rbac/permission/view/{$field->id}", "<i class='bi bi-eye'></i>", ['class' => 'btn btn-info btn-sm']) ."
	            	<button data-id='{$field->id}' class='btn btn-warning btn-sm btn-edit'><i class='bi bi-pencil'></i></button> 
	...
	public function actionSimpan($id = null)
	{
		...
		if ($post = $this->input->post('Permission')) {
			...
			if ($model) {
				...
				# Update parent name table auth child
				if ($save) {
					$save = $this->authitemchild->update(['parent' => $post['name']], ['parent' => $model->name]);
				}
