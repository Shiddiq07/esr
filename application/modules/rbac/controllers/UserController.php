...
class UserController extends CI_Controller {
	...
	public function actionGetData()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[] = $no;
            $row[] = $field->username;
            $row[] = $field->userDetail ? $field->userDetail->mergeFullName() : '-';
            $row[] = def($field, 'email', '-');
            ...
            $row[] = "
            	<div class='text-center'>
	            	...
	            	<a href='". base_url("/rbac/user/edit/{$field->id}") ."' class='btn btn-warning btn-sm'><i class='bi bi-pencil'></i></a> 
	            	<button data-id='{$field->id}' class='btn btn-danger btn-sm btn-delete'><i class='bi bi-trash'></i></button>
	            	<button data-id='{$field->id}' class='btn btn-primary btn-sm btn-add-detail' title='Tambah Detail'>
		...
		if ($post = $this->input->post()) {
			...
			if ($post['password_conf'] == $user['password']) {
				...
				try {
					...
					if ($save_user && !empty($user_group['group_id'])) {
						foreach ($user_group['group_id'] as $key => $value) {
	...
	public function actionGetAtasan()
	{
		...
		if (
			...
			$atasans = $this->userdetail->getListAtasans($id_unit);
	...
	public function actionEdit($id)
	{
		...
		$user_group = $this->groupuser->getGroupUser($user->id);

		if ($post = $this->input->post()) {
			$user_post = $post['User'];
			$user_group = $post['UserGroup']['group_id'];
			...
			try {
				...
				if ($save_user) {
					$this->groupuser->delete(['user_id' => $id]);
