...
<script src="<?= base_url('web/assets/vendor/nestable/jquery.nestable.js') ?>" type="text/javascript"></script>
<script type="text/javascript">
	...
	$(document).on('click', '.card-collapsed', function(event) {
		event.preventDefault();

		const cardBody = $(this).attr('href');
	...
	function appendMenu(id, url, label) {
	    var no = $('li.dd-item').length + 1;
	    $('.dd > .dd-list').append(
	        '<li class=\'dd-item\' data-id=\'' + id + '\'>' +
	        	'<a class="collapsed float-end p-3 card-collapsed" data-toggle="collapse" href="#' + id + '" aria-expanded="true" aria-controls="collapse-collapsed">' +
			        '<i class="bi bi-chevron-down"></i>' +
			    '</a>' + 
	            '<div class=\'dd-handle\'>' +
	                '<div class=\'card\'>' +
	                    '<div class=\'handle-head card-header\'>'+ label + '<span class=\'float-end\'><i>(' + url + ')</i></span> </div>' +
	                    '<div class=\'collapse dd-nodrag\' id=\'' + id + '\' role=\'tabpanel\' aria-labelledby=\'' + id + '\'>' +
                            '<div class=\'card\'>' +
